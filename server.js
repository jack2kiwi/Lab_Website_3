...
app.get('/team_stats', function(req, res) {
  ...
  .then(data => {
  	res.render('pages/team_stats',{
  			my_title: "Team Stats",
  			result_1: data[0],
  			result_2: data[1][0].count.toString(),
  			result_3: data[2][0].count.toString()
  		})
  })
  .catch(err => {
      // display error message in case an error
...
app.get('/player_info/select_player', function(req, res) {
  ...
  var player = 'SELECT * FROM football_players WHERE id = ' + player_id + ';';
  var numGames = 'SELECT COUNT(*) from football_games WHERE ' + player_id + ' = ANY(SELECT unnest(players));';
	db.task('get-everything', task => {
        return task.batch([
            task.any(allPlayers),
            task.any(player),
